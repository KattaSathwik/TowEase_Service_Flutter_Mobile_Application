...
import 'AllUserReqdetails.dart';
import 'UserReqDetailsForm.dart';
//import 'package:cloud_firestore/src/collection_reference.dart';

class User_Home_Page extends StatefulWidget {
  final String id;

  User_Home_Page({super.key, required this.id});

  //const Farmer({super.key, required String id});

  @override
  // ignore: no_logic_in_create_state
  State<User_Home_Page> createState() => _User_Home_Page_State(id: id);
}

class _User_Home_Page_State extends State<User_Home_Page> {
  String? id;
  var role;
  var email;
  var name;
  ...
  _User_Home_Page_State({required this.id});
    ...
    if (id != null && id!.isNotEmpty) {
      ...
      FirebaseFirestore.instance
        ...
        QuickAlert.show(
            context: context,
            title: "Success",
            text: "$name Logged in Successfully",
  ...
  List<Icon> optionIcons = [
    ...
    const Icon(Icons.request_page_outlined, color: Colors.white, size: 45),
    const Icon(Icons.miscellaneous_services_outlined, color: Colors.white, size: 45),
          ...
          child: SingleChildScrollView(
            child: Stack(
              children: [
                ...
                Padding(
                  padding: const EdgeInsets.only(top: 320.0, left: 15.0, bottom : 30.0),
                  child: Container(
                    width: 380,
                    height: 590,
                                ...
                                onTap: () {
                                  if (index == 0) {
                                    ...
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                        builder: (context) =>
                                            UserReqDetailsScreen(id: widget.id),
                                      ),
                                    );
                                  } else if (index == 3) {
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                        builder: (context) =>
                                            All_User_Req_Details_Page(id: widget.id),
                                      ),
                                    );
                                  } else if (index == 4) {
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                        builder: (context) =>
                                            All_User_Acc_Req_Details_Page(id: widget.id),
                                      ),
                                    );
                                  } else {
                                    Navigator.push(
